...
class AES {
   public:
    ...
    void ecbEncCounterMode(uint64_t baseIdx,
                           std::vector<block> &ciphertext) const {
        ecbEncCounterMode(toBlock(baseIdx), ciphertext.size(),
                          ciphertext.data());
    }

    void ecbEncCounterMode(block baseIdx,
                           std::vector<block> &ciphertext) const {
...
inline std::array<std::array<uint8_t, 4>, 4> &stateView(block &state) {
